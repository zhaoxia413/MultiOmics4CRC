...
otu_bl<-otu[,which(colnames(otu)%in%c("OTUid",meta_bl$SangerID))]
otu_treat<-otu[,which(colnames(otu)%in%c("OTUid",meta_treat$SangerID))]
...
otu_bl_limma<-data.frame(SangerID=colnames(otu_bl_limma),t(otu_bl_limma))

otu_treat_limma<-data.frame(row.names = otu_treat_limma$OTUid,otu_treat_limma[,-1])
otu_treat_limma<-data.frame(SangerID=colnames(otu_treat_limma),t(otu_treat_limma))


df_bl<-merge(meta_bl,otu_bl_limma,by="SangerID")
df_treat<-merge(meta_treat,otu_treat_limma,by="SangerID")
