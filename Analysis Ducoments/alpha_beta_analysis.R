...
"%ni%"<-Negate("%in%")
theme_set(theme_few())
meta<-fread("../../FBratio_paper/meta.csv",data.table = F)
alpha<-fread("../../FBratio_paper/alpha_diversity.csv",data.table = F)[,c(1:4)]
...
BL_treat<-fread("../../FBratio_paper/16pt_BL_treat_pairs_stool.csv",data.table = F)
...
paired_meta<-fread("../../FBratio_paper/stool_saliva_BL_paried.csv",data.table = F)
...
p5<-ggplot(alpha_paired,aes(Site,shannon,fill=Site))+
  ...
  scale_fill_d3()
##
load("../../FBratio_paper/regaMicrcobiome.RData")
...
tree_file <- read.tree("../../FBratio_paper/top500_OTU_phylo_tree.tre")
rooted_tree <- midpoint(tree_file)
otu<-fread("../../FBratio_paper/top500_OTU_table_phylo_tree.csv",data.table = F)
